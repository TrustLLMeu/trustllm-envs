# TrustLLM TGI environment

The container-based environment on using HuggingFace Text Generation
Inference (TGI), with example scripts for JSC and BSC systems.
...
1. Configure paths concerning the TGI environment to your liking
   in `trustllm-envs/tgi-env/configuration.sh`.
1. Switch into the TGI environment's directory:

   ```shell
   cd trustllm-envs/tgi-env
   ```
1. Build the TGI container:
