# TrustLLM EuroEval environment

The container-based environment on using EuroEval, with example
...
1. Configure paths concerning the EuroEval environment to your liking
   in `trustllm-envs/euroeval-env/configuration.sh`.
1. Switch into the EuroEval environment's directory:

   ```shell
   cd trustllm-envs/euroeval-env
   ```
1. Build the EuroEval container:
