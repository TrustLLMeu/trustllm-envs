...
# This script contains configuration specific to the EuroEval
...
apptainer_build_file="$scratch_dir"/apptainers/build/euroeval_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_image_tag=euroeval_"$(basename "$docker_image_uri")"_"$(uname -m)"
docker_build_container_name=build_euroeval_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
...
# accident.
apptainer_file="$scratch_dir"/apptainers/euroeval_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_container_name=euroeval_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
