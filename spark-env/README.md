# TrustLLM Apache Spark environment

The container-based environment on using Apache Spark with example
...
1. Configure paths concerning the Apache Spark environment to your
   liking in `trustllm-envs/spark-env/configuration.sh`.
1. Switch into the Apache Spark environment's directory:

   ```shell
   cd trustllm-envs/spark-env
   ```
1. Build the Apache Spark container:
