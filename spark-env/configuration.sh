...
# This script contains configuration specific to the Apache Spark
...
docker_image_tag=spark_"$(basename "$docker_image_uri")"_"$(uname -m)"
docker_build_container_name=build_spark_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
...
# accident.
apptainer_file="$scratch_dir"/apptainers/spark_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_container_name=spark_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
