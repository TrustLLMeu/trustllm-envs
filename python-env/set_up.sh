...
# be updated before re-installation.
...
if [ "$container_library" = apptainer ]; then
    _next_script="$(get_curr_dir)"/outside-container-scripts/set_up_apptainer.sh
    source "$_next_script" \
           __inherit__ "$_next_script" \
           "${_args[@]}"
elif [ "$container_library" = docker ]; then
    _next_script="$(get_curr_dir)"/outside-container-scripts/set_up_docker.sh
    source "$_next_script" \
           __inherit__ "$_next_script" \
           "${_args[@]}"
else
