...
# This script contains configuration specific to the Python
...
docker_image_tag=python_"$(basename "$docker_image_uri")"_"$(uname -m)"
docker_build_container_name=build_python_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
...
# accident.
apptainer_file="$scratch_dir"/apptainers/python_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_container_name=python_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
