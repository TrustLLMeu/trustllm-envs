# TrustLLM Megatron-LM environment

The container-based environment on using Megatron-LM, with example
...
1. Configure paths concerning the Megatron-LM environment to your
   liking in `trustllm-envs/megatron-env/configuration.sh`.
1. Switch into the Megatron-LM environment's directory:

   ```shell
   cd trustllm-envs/megatron-env
   ```
1. Build the Megatron-LM container:
...
1. Preprocess the validation data, tokenizing it and converting it to
   the binary format used by Megatron-LM. Here is an example assuming
