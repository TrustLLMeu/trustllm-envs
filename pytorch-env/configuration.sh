...
# This script contains configuration specific to the PyTorch
...
apptainer_build_file="$scratch_dir"/apptainers/build/pytorch_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_image_tag=pytorch_"$(basename "$docker_image_uri")"_"$(uname -m)"
docker_build_container_name=build_pytorch_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
...
# accident.
apptainer_file="$scratch_dir"/apptainers/pytorch_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)".sif
docker_container_name=pytorch_"$(basename "$docker_image_uri" | tr ':' '_')"_"$(uname -m)"
